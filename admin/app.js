...
(function ($) {
	...
	/**
	 * This function will update the status toggle button for wp and woo
    ...
    jQuery(".wpadder").on("click", function () {
        if (jQuery("#awraqnonce").val()) {
            let awraqNonce = jQuery("#awraqnonce").val();
        }
        ...
        jQuery.ajax({
            ...
            success: function (response) {
				...
				'<div class="table-def text-center shortcode">' + response.short_code + "</div>" +
				'<div class="table-def ">' +
				...
				'<button class="block px-8 py-4 border buttonborderradius" id="button' + response.id + '">Button</button>' +
				'</div></div>'+
    ...
    // Get all the shortcodes and Print
    if (jQuery("#awraqnonce").val()) {
        var awraqNonce = jQuery("#awraqnonce").val();
    }
    jQuery.ajax({
        url: ajaxurl,
        type: "post",
        ...
        success: function (response) {
            ...
            response.forEach(function (value, index, array) {
                ...
                let row =
                    ...
                    '<div class="table-def text-center shortcode">' + value.short_code + "</div>" +
                    '<div class="table-def ">' +
                    '<select name="selected_form" class="contact-7-selected">' + forms_select_options + "</select>" +
                    "</div>" +
                    '<div class="p-2 drawer-handle cursor-pointer"><svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 20 20" class="fill-current w-4 h-4 text-gray-400"><path d="M10 0a10 10 0 110 20 10 10 0 010-20zM2 10a8 8 0 1016 0 8 8 0 00-16 0zm10.54.7L9 14.25l-1.41-1.41L10.4 10 7.6 7.17 9 5.76 13.24 10l-.7.7z"/></svg>' +
	...
	/**
	 * This to autoload all the products
	 */
		...
		success: function(presponse){
			...
			if(presponse != false){
				...
				presponse.forEach(function(value, index, array){
					...
					let status = '<span class="bg-blue-100 px-2 py-1 rounded-full font-bold text-blue-900">Inactive</span>';
					if(value.selected != false){
						let status = '<span class="bg-blue-900 px-2 py-1 rounded-full font-bold text-white">Active</span>';
					}
					...
					let row =
						...
						'<div class="md:w-2/5 p-2 text-left shortcode"><span class="rounded-full bg-blue-100 text-blue-900 font-bold px-2 mr-1 py-1">'+value.id+'</span>' + value.title + "</div>" +
						...
						'</div>' +
						'<div class="md:w-1/5 text-center">' + status + '</div>'+
						'<div class=" md:w-1/5 p-2 drawer-handle cursor-pointer text-center"><button class="rounded flex ml-auto mr-0 px-8 py-2 mt-2 bg-blue-900 text-white text-xs tracking-wide font-medium psave p-'+value.id+'">Save</button>' +
						'</div></div>';
	...
	//save button : assigning button to product
	jQuery(".woo-appender").on("click",".psave",function(){
		...
		saveStatus = saveButtonToProduct(productId,buttonSelected);

		if(saveStatus != false){
			createPositiveMessage('Button assigned to Product');

		}else{
			createNegativeMessage('Button not assigned to Product!');
		}

	});

	//ajax to save button and product relation
	function saveButtonToProduct(productId = null , buttonId = null ){
		if(productId == null || buttonId == null){return false;}
	...
	//change button assignment status to active

    //drawer toggle - using delicate binding, since the handler is dynamically generated
